...
def parse_user_query(message: str, data_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    _drop_filter_duplicate_tags(entities, filters)
...
def _extract_vocab_matches(
    ...
    for tag, phrases in TAG_SYNONYMS.items():
        if any(
            _contains(text, phrase) and not _has_negation_near(text, phrase)
...
def _has_negation_near(text: str, normalized_phrase: str) -> bool:
    ...
    previous_two_words = " ".join(tokens[-2:])
    return previous_word in NEGATION_WORDS or previous_two_words in NEGATION_WORDS
...
def _drop_filter_duplicate_tags(entities: dict[str, Any], filters: dict[str, Any]) -> None:
    tags_to_drop: set[str] = set()
    if filters.get("max_spicy_level") is not None or filters.get("min_spicy_level") is not None:
        tags_to_drop.update({"không cay", "ít cay", "cay", "rất cay"})
    if filters.get("max_avg_delivery_time_min") is not None:
        tags_to_drop.add("giao nhanh")
    if filters.get("max_effective_price") is not None:
        tags_to_drop.add("rẻ")
    tags_to_drop.update({"ăn sáng", "ăn trưa", "ăn tối", "ăn khuya"})

    if tags_to_drop:
        entities["include_tags"] = [
            tag for tag in entities.get("include_tags", []) if tag not in tags_to_drop
        ]
        entities["exclude_tags"] = [
            tag for tag in entities.get("exclude_tags", []) if tag not in tags_to_drop
        ]
